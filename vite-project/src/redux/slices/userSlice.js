...
export const searchUsers = createAsyncThunk('users/search', async (query, { rejectWithValue }) => {
  try {
    ...
    const resp = await userApi.searchUsers ? await userApi.searchUsers(query) : await fetch(`/api/users/search?${params}`).then(r=>r.json())
...
export const fetchUser = createAsyncThunk('users/fetchById', async (id, { rejectWithValue }) => {
  try {
...
export const deleteUser = createAsyncThunk('users/delete', async (id, { rejectWithValue }) => {
  try {
    const data = await userApi.deleteUser ? await userApi.deleteUser(id) : await fetch(`/api/users/${id}`, { method: 'DELETE' }).then(r=>r.json())
    return { id, data }
  } catch (err) {
    return rejectWithValue(err.response?.data || { message: err.message })
  }
...
export const changeUserStatus = createAsyncThunk('users/status', async ({ id, action }, { rejectWithValue }) => {
  try {
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchUsers.rejected, (s,a)=>{ s.loading=false; s.error = a.payload || a.error })
      .addCase(searchUsers.fulfilled, (s,a)=>{ s.items = a.payload || [] })
      .addCase(fetchUser.fulfilled, (s,a)=>{ s.current = a.payload })
      .addCase(createUser.fulfilled, (s,a)=>{ s.items.unshift(a.payload) })
      .addCase(updateUser.fulfilled, (s,a)=>{ s.current = a.payload; s.items = s.items.map(u=> u._id===a.payload._id? a.payload : u) })
      .addCase(deleteUser.fulfilled, (s,a)=>{ s.items = s.items.filter(u=> u._id !== a.payload.id) })
      .addCase(changeUserStatus.fulfilled, (s,a)=>{ s.current = a.payload })
  }
})
