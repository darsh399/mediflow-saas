...
export default function VisitRecords() {
  ...
  return (
    <PageContainer>
      <PageHeader
        eyebrow="Field"
        title={employeeId ? 'Employee Visit History' : 'MR Visit Records'}
        description={employeeId ? 'Review visits for one employee by date range.' : 'Review employee activity without loading every visit record.'}
        actions={employeeId
          ? <button type="button" className="btn btn-ghost rounded-3" onClick={() => navigate('/admin/visits')}><i className="bi bi-arrow-left me-1" />Employee list</button>
          : <Link className="btn btn-primary rounded-3 fw-semibold" to="/mr/add-visit"><i className="bi bi-plus-lg me-1" />Add visit</Link>}
      />
