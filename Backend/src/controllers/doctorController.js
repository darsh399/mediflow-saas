...
export const createDoctor = async (req, res) => {
  try {
    ...
    const { name, clinicName, latitude, longitude, phone, specialty, email } = req.body;
...
export const updateDoctor = async (req, res) => {
  try {
    const id = req.params.id;
    const companyId = req.user?.companyId;
    const updated = await Doctor.findOneAndUpdate(companyId ? { _id: id, companyId } : { _id: id }, req.body, { new: true });
